...
class QuestionIndexViewTests(TestCase):
    ...
    def test_future_quetion_and_past_question(self):
        creat_question(question_text='Future question.', days=30)
        creat_question(question_text='Past question.', days=-30)
        response = self.client.get(reverse('poll:index'))
        self.assertQuerysetEqual(response.context['latest_question_list'], ['<Question: Past question.>'])
...
class QuestionDetailViewTests(TestCase):
    ...
    def test_future_question(self):
        future_question = creat_question(question_text="Future qeustion.", days=5)
        url = reverse('poll:detail', args=(future_question.id,))
        response = self.client.get(url)
        self.assertEqual(response.status_code, 404)

    def test_past_question(self):
        past_question = creat_question(question_text="Past qustion.", days=-5)
